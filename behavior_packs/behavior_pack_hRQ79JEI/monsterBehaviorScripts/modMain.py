...
@Mod.Binding(name='fireSeedMod', version="1.0")
class TutorialMod(object):

    # 类的初始化函数
    def __init__(self):
        print "===== init fireSeed mod ====="

    # InitServer绑定的函数作为服务端脚本初始化的入口函数，通常是用来注册服务端系统system和组件component
    @Mod.InitServer()
    def initServer(self):
        print "===== init fireSeed server ====="
        ...
        serverApi.RegisterSystem('fireSeedMod', 'fireSeedServerSystem',
                                 "monsterBehaviorScripts.fireSeedServerSystem.FireSeedServerSystem")

    # DestroyServer绑定的函数作为服务端脚本退出的时候执行的析构函数，通常用来反注册一些内容,可为空
    @Mod.DestroyServer()
    def destroyServer(self):
        print "===== destroy fireSeed server ====="
    ...
    def initClient(self):
        ...
        # 取名名称尽量个性化，不能与其他人的MOD冲突，可以使用英文、拼音、下划线这三种。
        clientApi.RegisterSystem('fireSeedMod', 'fireSeedClientSystem',
                                 "monsterBehaviorScripts.fireSeedClientSystem.FireSeedClientSystem")
